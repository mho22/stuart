...
On an open item **nobody is assigned to** — no assignee, no requested reviewer, no standing suggestion — the steward suggests **two people**: a first reviewer, and a backup for availability — each with the *why*. It weighs **continuity** (the same reviewer follows a saga) against **load-balancing** (spread one-offs), reading the `maintainers/` profiles.

**Open concerns never hold the suggestion back.** Coverage is patrolled on every open item, whatever else the steward has to say about it. Rules 1–3 once gated this one, and the gate failed in both directions: it let the steward decide for itself that a concern was worth blocking a review over, and since a concern clears only when the PR changes, a judgment its author disagreed with sat on the item indefinitely and coverage never ran at all. Whether a concern should hold up a review is a conversation between the maintainer and the contributor — who, in shadow mode, has not even heard the concern unless a human chose to paste it. Concern and suggestion travel in one message instead: the concern leads, the two names close it.
...
**The message is written for the suggested reviewer to post, not about them.** In shadow mode a message only reaches the thread when a human pastes it, and the person holding a reviewer card is the suggested reviewer themselves — so the body is the first-person comment they could post verbatim: claiming the review, grounding it in their own linked work, naming the backup. Pasting it is how the human decides (steward's rule 6); a message its own assignee cannot use has failed. Where the item already carries a judgment the suggestion joins that message instead of becoming a second one, which costs the first person; the two names and the reason each fits are what must survive.

*Grounded in: the brainstorm's policy decisions (continuity vs. load-balancing, delivered as a message); the two-name form is an amendment of 2026-07-27, recorded in `specification/guidelines/DESIGN.md`; coverage-based rather than creation-only is an amendment of 2026-08-19, recorded in `specification/workflow/DESIGN.md` — the shadow-mode record is private, so a "new item" webhook moment the steward could act on never exists; the brainstorm's validity gate is dropped by an amendment of 2026-09-09, recorded in `specification/guidelines/DESIGN.md` — the same privacy makes a steward-enforced block a demand the contributor never received.*
...
2. **One story per comment, one comment per item.** Lead with the point, evidence in a few sentences, then stop. Write at the length of a busy maintainer's note: one short paragraph most of the time, three at the very most, and never restate what the thread already says. An item leaves with exactly one message however many rules had something to say about it, because two files on one item is two comments on one thread, and that is the machine-written shape this rulebook exists to avoid; compose the single message from the start rather than writing two and merging them afterwards. If a comment about cognitive load is tiring to read, the steward has refuted itself.
