...
**Return.** One updated `maintainers/<login>.md` per roster member, obeying MAINTAINING.md; a refreshed `.claude/state/orbit.json`; an analysis report through the store that **opens with one line per roster member** — what their window showed, linking the profile and the evidence behind the claim — because this workflow's judgments are people, not items, and a member who was profiled but goes unnamed in the report is a member the maintainer never hears about; boundary and classification calls come after those lines, never instead of them; and whose ledger counts searches made (baseline + follow-ups), repos classified/admitted/dropped, and off-project items dropped unread; the analysis timestamp advanced last.
