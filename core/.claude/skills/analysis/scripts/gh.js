#!/usr/bin/env node
// gh.js — read-only GitHub helper for the analysis skill's scripts.
// SIBLING COPIES: this helper is intentionally duplicated per skill so each
// skill's allowed-tools sandbox stays confined to its own scripts/. If you change
// the GET-only discipline here, change it in sweep/scripts/gh.js too — they are
...
// Unauthenticated GitHub is 60 requests/hour, far under one analysis window, so no
// token at all fails loudly here rather than as a wall of 403s halfway through a run.
//
...
// Search issues/PRs globally (person-first). Returns items array.
//
...
async function searchIssues(q, { perPage = 100, maxPages = 3 } = {}) {
  const hasType = /(^|\s)(type|is):(issue|pr|pull-request)\b/i.test(q);
  const queries = hasType ? [q] : [`${q} type:issue`, `${q} type:pr`];
  const items = [];
  for (const query of queries) {
    for (let page = 1; page <= maxPages; page++) {
      const res = await ghApi('search/issues', {
        fields: { q: query, advanced_search: 'true', per_page: String(perPage), page: String(page) },
      });
