...
**The main context is also the most expensive place to run anything.** Every command it runs replays its whole session at model prices, and by step 3 that session is large. So it runs each step-1 script once, spawns each wave of batches in one message, and runs nothing between the spawn and the returns — no counting, no progress checks, no per-item verification. Every question of that kind belongs to `reconcile.js`, once, in step 5: the 2026-09-23 run answered them with ~280 repeated `ls` and `find` calls instead and spent more on those than on judgment.
...
Write every date in a message absolutely — "on 2026-09-22", never "yesterday" or "last week" — so a message that is still right does not rot into a rewrite. On a re-verify, republish only when the verdict or the evidence changed: wording alone never justifies a republish, and the entry refresh through `write-cache.js` is the whole update. The 2026-09-23 run rewrote 69 files, most because a relative date had rotted, and flipped no verdict.
...
**5. Housekeeping and close.** Before the report, run `reconcile.js` once. It answers everything the close-out asks in one call: coverage in both directions, every open item carrying no message and every item carrying two, cache health (missing entries, orphans, message pointers that resolve to nothing), the voice checks (an em dash in a body, a message linking its own item), and the kind counts for the ledger. Finish what it names rather than reporting around it — an uncovered item goes back through step 2; a doubled item gets folded per *Coverage*, which means a composition step went wrong earlier and is worth saying in the report — then run it once more to confirm `clean`. Never answer any of its questions with your own `ls`, `find`, or `grep`, and take the ledger's numbers from its output. The report is prose read by a person, so it obeys `.claude/docs/human-tone.md` the same as any message; the template gives it its sections, the tone guide governs every sentence inside them.

```bash
node .claude/skills/sweep/scripts/reconcile.js --file /tmp/open.json      # coverage, cache, voice, kind counts — once
...
- **Fewer messages than open items** — the run is not finished. `reconcile.js` names the uncovered items in `without_message`; send them back through step 2. Never close the gap by lowering the expectation, and never resolve it by trimming what other batches wrote.
- **More messages than open items** — an item carries two; `reconcile.js` names it in `with_two`. Fold each pair into the kind the precedence in *Coverage* names, and check the batch prompts: the usual cause is a reviewer or reminder step that did not `recall` the item first.
