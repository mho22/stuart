...
7. **Two suggested reviewers.** A first reviewer and an availability backup. Coverage-based and ungated — open concerns never hold the suggestion back — the author never their own reviewer in either role.
...
- **2026-09-09 (maintainer decision)** — Concerns are non-blocking: rule 7's validity gate is dropped, and an open `concerns` message no longer withholds the reviewer suggestion. The gate had the steward deciding for itself when a concern was worth blocking a review, and a concern clears only when the PR changes — so a judgment its author disagreed with sat on the item indefinitely and coverage never ran, on a PR whose contributor had not heard the concern at all, the record being private. Asking a contributor to answer a concern before review is the maintainer's conversation to have with them; the steward's job ends at raising it. Concern and suggestion now travel in one message, the concern leading (workflow/DESIGN.md, "Sweep"; sweep/SKILL.md step 3). Surfaced by PR #4280, where a design question the author considered settled had silenced rule 7 with no way to say so.
