...
- **Search** — case-insensitive substring over `number + #number + kind + title + summary + body` (an amendment of 2026-09-09: the number is stored bare, so typing the natural `#4280` matched all 21 messages *citing* that PR in prose and dropped the one message *about* it — a message's own body never writes its own number. `summary` joined at the same time; it renders on the expanded card, so searching it can no longer surface a card with nothing visibly matching).
...
- **Maintainer chips** — "All maintainers" pinned and carrying no count (it is the one chip without a number), then one chip per roster member *actually mentioned* in at least one message, in a horizontally scrolling row (scrollbar hidden on every engine). A message "mentions" a maintainer when the handle appears in the body — `@foo` or bare `foo` — matched case-insensitively with lookarounds rejecting word or hyphen characters on either side, so `adamziel` never matches inside `adamziel-fork`.
